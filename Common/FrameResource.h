	...
	public:
		...
		void CreateConstantBufferView(D3D12_CPU_DESCRIPTOR_HANDLE cpuDescHandle, INT offset = 0)
		{
			if (std::is_same<StructureBuffer, TConstantType>::value)
				return;
			UINT ConstantBufferByteSize = AppUtil::CalcConstantBufferByteSize(sizeof(TConstantType));
			...
			cbvDesc.SizeInBytes = AppUtil::CalcConstantBufferByteSize(sizeof(TConstantType));
