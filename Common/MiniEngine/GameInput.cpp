...
#define WinRTKey(name) (unsigned char)Windows::System::VirtualKey::name
    ...
    void KbmInitialize()
    {
        ...
        if (FAILED(s_Keyboard->SetCooperativeLevel(DX::GWindow, DISCL_FOREGROUND | DISCL_NONEXCLUSIVE)))
        ...
        if (FAILED(s_Mouse->SetCooperativeLevel(DX::GWindow, DISCL_FOREGROUND | DISCL_EXCLUSIVE)))
    ...
    void KbmUpdate()
    {
        ...
        if (foreground != DX::GWindow // wouldn't be able to acquire
            || !visible)
        {
            KbmZeroInputs();
        }
